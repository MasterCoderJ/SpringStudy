<%@page import="com.edu.shopprac.domain.Product"%>
              ...
              <div class="card-body table-responsive p-0">
                <table class="table table-hover text-nowrap">
                  ...
                  <%Product product= productList.get(i); %>
                    <tr>
                      <td><%=i %></td>
                      <td><a href="/admin/product/detail?product_idx=<%=product.getProduct_idx()%>"><%=product.getCategory().getCategory_name() %></a></td>
	...
	<script type="text/javascript">
		...
		function preview(files){
			...
			for(let i=0; i<files.length; i++){
				...
				if(checkDuplicate(file.name)<1){ //중복된 이미지가 없을때만 실행
					let reader= new FileReader(); //스트림 생성
					reader.onload=(e)=>{
						
						key++; //사용자가 이미지를 선택할때마다 1씩 증가하여 중복을 불허함
						
						let json= []; //imageList배열에 복합적인 정보르 담아놓기 위해
						json['key']=key; //추후 이미지 삭제시 기준값
						json['name']=file.name; //중복이미지 추가 방지
		...
		*/
		function regist(){
			...
			$.ajax({
				url:"/admin/rest/product", //rest는 명사적표현
				method:"post",
				data:formData,
				processData:false, //queryString 사용여부

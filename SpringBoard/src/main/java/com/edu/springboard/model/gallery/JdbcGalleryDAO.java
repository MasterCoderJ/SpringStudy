...
public class JdbcGalleryDAO implements GalleryDAO {
	...
	public List selectAll() {
		...
		List galleryList = jdbcTemplate.query(
				"select * from gallery order by gallery_idx",
				new RowMapper<Gallery>() {

					public Gallery mapRow(ResultSet rs, int rowNum)
							throws SQLException {
						...
						List photoList = jdbcTemplate.query(sql,
								new Object[]{gallery.getGallery_idx()},
								new RowMapper<Photo>() {
	...
	public void insert(Gallery gallery) throws GalleryException {
		...
		int result = jdbcTemplate.update(sb.toString(), new Object[]{
				gallery.getTitle(), gallery.getWriter(), gallery.getContent()});

		// mybatis 처럼 insert 와 동시에, pk 얻기
		String sql = "select seq_gallery.currval as gallery_idx from dual";

		int gallery_idx = jdbcTemplate.queryForObject(sql,
	...
	public void update(Gallery gallery) throws GalleryException {
		...
		int result = jdbcTemplate.update(sb.toString(),
				new Object[]{gallery.getTitle(), gallery.getWriter(),
						gallery.getContent(), gallery.getGallery_idx()});
		if (result < 1) {
